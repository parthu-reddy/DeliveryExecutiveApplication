            ...
            for (String orderIdStr : orderIds) {
                try {
                    ...
                    if (failedCycles >= 5) {
                        ...
                        org.springframework.messaging.Message<String> message = org.springframework.messaging.support.MessageBuilder.withPayload(objectMapper.writeValueAsString(eventPayload)).setHeader(org.springframework.kafka.support.KafkaHeaders.TOPIC, com.fooddelivery.common.constants.KafkaConstants.TOPIC_ORDER_EVENTS).setHeader(org.springframework.kafka.support.KafkaHeaders.KEY, orderIdStr).setHeader("eventType", com.fooddelivery.common.constants.EventType.MANUAL_INTERVENTION_REQUIRED.name()).build();
